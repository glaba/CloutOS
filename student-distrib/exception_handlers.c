...
int line1_##handler_name() {line1_gen return 0;} \
int line2_##handler_name() {line2_gen return 0;} \
...
// Divide by zero is a special case since we have to send a special signal
void divide_zero_handler() {
	if (in_userspace) {
		send_signal(get_pid(), SIGNAL_DIV_ZERO, 0);
		handle_signals();
	} else {
		print_error("DIVIDE BY ZERO EXCEPTION\n", NULL, NULL, 0);
		while (1);
	}
}
